...
export default function Header() {
  ...
  return (
    <>
      <ContactModal open={state.open}
setOpen={(v) => dispatch({open: v})}/>
      <header className="relative">
        <div className="contents"><a href="https://www.trustpilot.com/review/zizoo.com"
target="_blank"
                          ...
                          <Menu.Item key={item.name}>
                            {({active}) => (
                              <a
                                href={item.href}
                                className={
                                  (active ? 'bg-gray-100' : '') + ' block py-2 px-4 text-sm text-gray-700'
                                }
                              >
                                {item.name}
                              </a>
