import Newsletter from '../home/newsletter';
import Guides from '../home/guides';
import Questions from '../home/questions';
import Link from '../helper/link';

export default function Footer() {
  return (
    <>
      <Questions />
      <Guides />
      <Newsletter />
      <footer className="pt-10 bg-gray-100">
        <div className="container max-w-screen-xl mx-auto px-4 2xl:px-0">
          <div className="grid grid-flow-row grid-cols-1 gap-8 pb-8 md:gap-4 md:grid-cols-2 lg:grid-cols-4">
            <div className="flex-1">
              <h4 className="inline-block mb-2 text-sm font-semibold text-blue-500">Zizoo</h4>
              <ul className="flex flex-col space-y-2 text-sm text-blue-500">
                <li>
                  <Link
                    href="/en/about-zizoo/"
                  >
                    <a
                      className="flex flex-1 text-sm text-blue-500"
                    >
                      About Us
                    </a>
                  </Link>
                </li>
                <li>
                  <a href="/en/">EN</a>
                  {' '}
                  |
                  {' '}
                  <a href="/en-us/">EN-US</a>
                  {' '}
                  |
                  {' '}
                  <a href="/de/">DE</a>
                  {' '}
                  |
                  {' '}
                  <a
                    href="/nl/"
                  >
                    NL
                  </a>
                  {' '}
                  |
                  {' '}
                  <a href="/it/">IT</a>
                  {' '}
                  |
                  {' '}
                  <a href="/es/">ES</a>
                  {' '}
                  |
                  {' '}
                  <a href="/fr/">FR</a>
                </li>
              </ul>
            </div>
            <div className="flex-1 lg:-ml-6">
              <h4
                className="inline-block mb-2 text-sm font-semibold text-blue-500"
              >
                Factual and
                Legal pages
              </h4>
              <ul className="flex flex-col space-y-2 text-sm text-blue-500">
                <li>
                  <a
                    href="https://help.zizoo.com/s/?language=en_US"
                    className="flex flex-1 text-sm text-blue-500"
                  >
                    Support
                  </a>
                </li>

              </ul>
            </div>
            <div className="flex-1 lg:-ml-6">
              <h4
                className="inline-block mb-2 text-sm font-semibold text-blue-500"
              >
                Contact
                Us
              </h4>
              <ul className="flex flex-col space-y-2 text-sm text-blue-500">
                <li>
                  <a
                    href="https://help.zizoo.com/s/contactsupport?language=en_US"
                    className="flex flex-1 text-sm text-blue-500"
                  >
                    Contact
                  </a>
                </li>
              </ul>
              <div className="flex flex-col mt-2 space-y-4">
                <ul className="flex flex-col space-y-2 text-sm text-blue-500">
                  <li>Mon-Sun: 9-9pm CET</li>
                  <li>
                    <a
                      href="tel:UK: +33 1 76 44 06 49"
                      itemProp="telephone"
                      title="Give us a call during opening hours"
                      aria-label="Phone nr."
                    >
                      UK: +33 1 76 44 06 49
                    </a>
                  </li>
                  <li>
                    WhatsApp:
                    <a
                      target="_blank"
                      rel="noreferrer noopener"
                      href="https://api.whatsapp.com/send?phone=+49 15731910983"
                      aria-label="whatsapp"
                    >
                      +49
                      15731910983
                    </a>
                  </li>
                  <li>
                    <div>
                      Zizooboats GmbH
                      <br />
                      Ohlauer Str. 9, 10999 Berlin
                      <br />
                      Germany
                    </div>
                  </li>
                </ul>
              </div>
            </div>
            <div className="flex flex-col space-y-4">
              <h4
                className="inline-block text-sm font-semibold text-blue-500"
              >
                Social
                Media
              </h4>
              <ul className="flex flex-row space-x-4 text-sm text-blue-500">
                <li>
                  <a
                    className="flex items-center p-1 border-2 rounded-full border-blue-500"
                    href="https://www.pinterest.com/zizooboats/"
                    target="_blank"
                    rel="noreferrer noopener"
                    aria-label="Pinterest"
                  >
                    <svg
                      fill="currentColor"
                      viewBox="0 0 512 512"
                      xmlns="http://www.w3.org/2000/svg"
                      className="w-4 h-4"
                    >
                      <path
                        d="m222 338c-9 44-19 80-29 108c-11 28-28 50-50 66c-2-25-3-46-3-61c-1-16 1-35 5-57c5-22 8-39 12-53c3-14 8-31 13-51c5-20 10-37 12-50c-8-18-11-38-9-60c1-22 9-40 24-54c15-14 32-18 51-10c13 5 21 15 22 30c2 15-1 31-6 49c-6 17-12 35-17 52c-5 18-6 33-2 46c4 14 15 22 33 25c23 5 43-1 61-19c19-18 31-40 38-67c8-27 9-55 6-84c-3-28-13-50-28-65c-21-22-47-34-77-36c-30-2-58 4-83 17c-26 13-46 33-62 60c-16 27-21 56-16 87c1 7 5 16 11 27c6 10 10 18 11 24c1 5-2 17-9 34c-50-12-74-51-72-118c2-47 20-87 54-120c35-32 74-52 119-57c56-6 106 4 149 29c43 26 67 63 74 112c9 60-3 115-34 163c-32 48-75 70-129 66c-8-1-15-3-22-5c-7-2-12-5-15-7c-3-2-9-5-16-10c-7-5-12-9-16-11"
                        stroke="currentColor"
                        strokeWidth=".9"
                        strokeMiterlimit="10"
                      />
                    </svg>
                  </a>
                </li>
                <li>
                  <a
                    className="flex items-center p-1 border-2 rounded-full border-blue-500"
                    href="https://www.instagram.com/zizoo_boats/"
                    target="_blank"
                    rel="noreferrer noopener"
                    aria-label="Instagram"
                  >
                    <svg
                      fill="currentColor"
                      viewBox="0 0 512 512"
                      xmlns="http://www.w3.org/2000/svg"
                      className="w-4 h-4"
                    >
                      <path
                        d="m389 256c0-9-1-17-3-26c0 0 90 0 90 0c0 0 0 177 0 177c0 19-6 35-20 49c-13 13-29 20-48 20c0 0-304 0-304 0c-19 0-35-7-48-20c-14-14-20-30-20-49c0 0 0-177 0-177c0 0 89 0 89 0c-1 11-2 20-2 26c0 36 13 67 39 94c26 26 57 39 94 39c36 0 67-13 94-39c26-27 39-58 39-94m19-220c19 0 35 6 48 20c14 13 20 29 20 48c0 0 0 75 0 75c0 0-111 0-111 0c-27-37-63-56-109-56c-47 0-83 19-109 56c0 0-111 0-111 0c0 0 0-75 0-75c0-19 6-35 20-48c13-14 29-20 48-20c0 0 304 0 304 0m32 85c0 0 0-37 0-37c0-8-4-12-12-12c0 0-37 0-37 0c-8 0-12 4-12 12c0 0 0 37 0 37c0 3 1 5 3 8c3 3 6 4 9 4c0 0 37 0 37 0c8 0 12-4 12-12m-102 135c0 23-8 42-24 58c-16 16-35 24-58 24c-23 0-42-8-58-24c-16-16-24-35-24-58c0-23 8-43 24-58c16-16 35-24 58-24c23 0 42 8 58 24c16 15 24 35 24 58"
                        stroke="currentColor"
                        strokeWidth=".9"
                        strokeMiterlimit="10"
                      />
                    </svg>
                  </a>
                </li>
                <li>
                  <a
                    className="flex items-center p-1 border-2 rounded-full border-blue-500"
                    href="https://www.facebook.com/ZizooBoats"
                    target="_blank"
                    rel="noreferrer noopener"
                    aria-label="Facebook"
                  >
                    <svg
                      viewBox="0 0 11 23"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                      className="w-4 h-4"
                    >
                      <path
                        fillRule="evenodd"
                        clipRule="evenodd"
                        d="M7.31471 23H2.43724V11.4985H0V7.53553H2.43724V5.15627C2.43724 1.92344 3.81271 0 7.72302 0H10.9776V3.96447H8.94351C7.42126 3.96447 7.32067 4.51827 7.32067 5.55185L7.31396 7.53553H11L10.5686 11.4985H7.31396V23H7.31471Z"
                        fill="currentColor"
                      />
                    </svg>
                  </a>
                </li>
                <li>
                  <a
                    className="flex items-center p-1 border-2 rounded-full border-blue-500"
                    href="https://www.youtube.com/zizooboats"
                    target="_blank"
                    rel="noreferrer noopener"
                    aria-label="YouTube"
                  >
                    <svg
                      fill="currentColor"
                      viewBox="0 0 512 512"
                      xmlns="http://www.w3.org/2000/svg"
                      className="w-4 h-4"
                    >
                      <path
                        d="m314 355l0 61c0 12-4 19-11 19c-5 0-9-2-13-6l0-86c4-5 8-7 13-7c7 0 11 7 11 19z m97 1l0 13l-26 0l0-13c0-13 4-20 13-20c8 0 13 7 13 20z m-276-63l30 0l0-26l-89 0l0 26l30 0l0 163l29 0z m82 163l25 0l0-141l-25 0l0 108c-6 8-11 12-16 12c-4 0-6-2-6-6c-1-1-1-4-1-10l0-104l-25 0l0 112c0 9 1 16 2 20c2 7 8 11 17 11c9 0 19-6 29-17z m122-42l0-57c0-13 0-23-2-28c-3-11-10-16-20-16c-10 0-19 5-27 16l0-62l-25 0l0 189l25 0l0-14c9 11 17 16 27 16c10 0 17-5 20-16c2-5 2-14 2-28z m97-3l0-4l-26 0c0 10 0 16-1 18c-1 6-5 10-11 10c-9 0-13-7-13-20l0-25l51 0l0-29c0-15-3-26-8-33c-7-10-17-15-30-15c-13 0-23 5-31 15c-5 7-8 18-8 33l0 49c0 15 3 26 9 33c7 10 17 15 31 15c13 0 24-5 30-15c4-5 6-10 6-16c1-1 1-7 1-16z m-174-261l0-60c0-13-4-20-12-20c-8 0-12 7-12 20l0 60c0 13 4 20 12 20c8 0 12-7 12-20z m206 215c0 44-3 77-8 100c-2 11-8 20-16 28c-9 7-18 12-29 13c-35 4-88 6-159 6c-71 0-124-2-159-6c-11-1-20-6-29-13c-8-8-14-17-16-28c-5-22-8-55-8-100c0-45 3-78 8-100c2-12 8-21 16-29c9-7 18-12 30-13c35-4 87-6 158-6c71 0 124 2 159 6c11 1 20 6 29 13c8 8 14 17 16 29c5 21 8 54 8 100z m-285-365l29 0l-35 114l0 77l-28 0l0-77c-3-14-9-34-18-61c-7-19-13-37-18-53l30 0l20 75z m105 95l0 50c0 16-2 27-8 34c-7 10-17 14-30 14c-13 0-23-4-30-14c-5-7-8-19-8-34l0-50c0-15 3-26 8-33c7-10 17-15 30-15c13 0 23 5 30 15c6 7 8 18 8 33z m96-46l0 142l-26 0l0-15c-10 12-20 17-29 17c-9 0-15-3-17-10c-2-5-3-12-3-22l0-112l26 0l0 105c0 6 1 9 1 10c0 4 2 6 6 6c5 0 10-4 16-12l0-109z"
                        stroke="currentColor"
                        strokeWidth=".9"
                        strokeMiterlimit="10"
                      />
                    </svg>
                  </a>
                </li>
                <li>
                  <a
                    className="flex items-center p-1 border-2 rounded-full border-blue-500"
                    href="https://twitter.com/ZizooBoats"
                    target="_blank"
                    rel="noreferrer noopener"
                    aria-label="Twitter"
                  >
                    <svg
                      fill="currentColor"
                      viewBox="0 0 512 512"
                      xmlns="http://www.w3.org/2000/svg"
                      className="w-4 h-4"
                    >
                      <path
                        d="m492 110c-13 18-29 35-49 50c0 0 0 12 0 12c0 44-10 87-30 128c-21 41-53 76-96 104c-43 28-92 42-148 42c-55 0-104-14-149-43c5 1 13 1 24 1c45 0 85-13 120-40c-21-1-40-8-56-20c-17-12-28-28-34-48c3 1 9 2 17 2c9 0 18-1 26-3c-23-5-41-16-56-34c-14-18-21-38-21-61c0 0 0-1 0-1c12 6 27 11 43 12c-29-20-43-47-43-81c0-16 4-32 13-48c53 64 119 98 200 100c-2-6-3-13-3-21c0-27 9-50 28-68c19-19 42-28 69-28c28 0 51 9 70 29c20-4 41-11 61-22c-7 22-21 40-42 53c19-3 38-8 56-15"
                        stroke="currentColor"
                        strokeWidth=".9"
                        strokeMiterlimit="10"
                      />
                    </svg>
                  </a>
                </li>
              </ul>
              <h4 className="inline-block text-sm font-semibold text-blue-500">Secure payments handled by</h4>
              <div className="flex flex-row space-x-4">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                  viewBox="0 0 51 34"
                  className="object-contain w-auto h-10"
                >
                  <rect
                    width="50.999"
                    height="33.028"
                    fill="#000"
                    rx="5.625"
                  />
                  <g>
                    <path
                      fill="#fff"
                      fillRule="evenodd"
                      d="M38.58 28.558c0-.545.336-.981.877-.981.518 0 .859.418.859.98s-.341.982-.859.982c-.541 0-.876-.436-.876-.981zm2.278 0v-2.61h-.565v1.514c-.194-.26-.511-.424-.911-.424-.789 0-1.389.636-1.389 1.52s.6 1.52 1.389 1.52c.4 0 .717-.164.911-.424v.351h.565zm-7.953 0c0-.545.335-.981.876-.981.517 0 .859.418.859.98s-.342.982-.86.982c-.54 0-.876-.436-.876-.981zm2.277 0V27.11h-.565v.352c-.194-.26-.511-.424-.911-.424-.789 0-1.389.636-1.389 1.52s.6 1.52 1.389 1.52c.4 0 .717-.164.911-.424v.351h.565zm6.645 1.23v.072h.064a.06.06 0 00.035-.01c.009-.005.013-.014.013-.026a.03.03 0 00-.013-.027.064.064 0 00-.035-.009zm.064-.05c.033 0 .06.008.077.023a.077.077 0 01.027.063c0 .022-.007.04-.021.054a.102.102 0 01-.062.026l.085.101h-.066l-.079-.1h-.025v.1h-.056v-.267zm-.017.359c.03 0 .059-.006.085-.018a.223.223 0 00.131-.206.229.229 0 00-.063-.159.212.212 0 00-.238-.047.213.213 0 00-.116.119.227.227 0 00-.017.087.226.226 0 00.063.158.23.23 0 00.07.048.21.21 0 00.085.018zm.002-.511a.276.276 0 01.2.084.295.295 0 01.084.203.284.284 0 01-.084.202.293.293 0 01-.09.062.275.275 0 01-.313-.062.29.29 0 010-.405.278.278 0 01.203-.084zm-4.53-2.548a.863.863 0 00-.765.424v-.352h-.559v2.895h.565v-1.629c0-.478.224-.787.623-.787.124 0 .253.018.383.073l.188-.545a1.275 1.275 0 00-.435-.08zm-21.704 1.52c0-.545.336-.981.877-.981.518 0 .859.418.859.98s-.341.982-.86.982c-.54 0-.876-.436-.876-.981zm2.278 0V27.11h-.565v.352c-.194-.26-.512-.424-.912-.424-.788 0-1.388.636-1.388 1.52s.6 1.52 1.388 1.52c.4 0 .718-.164.912-.424v.351h.565zm7.61-.981c.418 0 .7.278.742.733h-1.518c.053-.424.324-.733.777-.733zm.011-.54c-.824 0-1.388.612-1.388 1.515 0 .92.588 1.526 1.43 1.526.423 0 .81-.109 1.152-.406l-.3-.43a1.315 1.315 0 01-.817.303c-.395 0-.794-.23-.865-.75h2.1a3.17 3.17 0 00.011-.243c-.005-.903-.535-1.514-1.323-1.514zm3.952 1.521c0 .878.57 1.52 1.477 1.52.423 0 .706-.097 1.011-.345l-.294-.454c-.23.17-.47.26-.735.26-.488-.006-.87-.4-.87-.981s.382-.975.87-.981c.265 0 .506.09.735.26l.294-.454c-.305-.248-.588-.345-1.011-.345-.906 0-1.477.642-1.477 1.52zm-8.527-1.217c-.294-.2-.7-.303-1.147-.303-.712 0-1.147.345-1.147.92 0 .473.317.758.947.848l.288.043c.335.048.518.17.518.333 0 .224-.248.37-.665.37-.441 0-.73-.14-.935-.304l-.289.455c.418.315.918.375 1.218.375.812 0 1.259-.387 1.259-.939 0-.508-.341-.769-.96-.86l-.287-.042c-.265-.036-.5-.121-.5-.315 0-.212.223-.357.558-.357.36 0 .706.14.877.248zm7.886-.303a.863.863 0 00-.764.424v-.352h-.559v2.895h.565v-1.629c0-.478.223-.787.623-.787.124 0 .253.018.382.072l.189-.545a1.272 1.272 0 00-.436-.078zm-5.216.072h-1.024v-.878h-.57v.878h-.594v.527h.594v1.37c0 .671.23 1.071.953 1.071.264 0 .57-.085.764-.224l-.176-.515c-.182.11-.383.164-.541.164-.306 0-.43-.194-.43-.485v-1.38h1.024zm-9.164 2.895v-1.823c0-.684-.4-1.138-1.082-1.144-.359-.006-.73.109-.988.514-.195-.32-.5-.514-.93-.514-.3 0-.617.084-.847.424v-.352h-.564v2.895h.57v-1.66c0-.502.294-.768.712-.768.406 0 .635.272.635.763v1.665h.57v-1.66c0-.502.307-.768.713-.768.417 0 .64.272.64.763v1.665z"
                      clipRule="evenodd"
                    />
                  </g>
                  <g
                    fillRule="evenodd"
                    clipRule="evenodd"
                  >
                    <path
                      fill="#ff5f00"
                      d="M20.308 22.201h10.18V5.081h-10.18z"
                    />
                    <path
                      fill="#eb001b"
                      d="M21.358 13.64c0-3.473 1.58-6.566 4.04-8.56a10.338 10.338 0 00-6.536-2.328c-5.84 0-10.574 4.875-10.574 10.889 0 6.013 4.734 10.888 10.574 10.888 2.467 0 4.736-.87 6.535-2.328-2.46-1.994-4.039-5.087-4.039-8.56z"
                    />
                    <path
                      fill="#f79e1b"
                      d="M42.172 20.495v-.53h-.108l-.123.365-.123-.365h-.107v.53h.075v-.4l.116.345h.078l.116-.345v.4zm-.68 0v-.44h.138v-.09h-.35v.09h.138v.44zm1.013-6.855c0 6.014-4.734 10.89-10.573 10.89-2.467 0-4.737-.87-6.535-2.329 2.46-1.994 4.039-5.087 4.039-8.56s-1.58-6.567-4.04-8.56a10.338 10.338 0 016.536-2.329c5.84 0 10.573 4.875 10.573 10.889z"
                    />
                  </g>
                </svg>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                  viewBox="0 0 51 34"
                  className="object-contain w-auto h-10"
                >
                  <rect
                    width="51"
                    height="33.029"
                    fill="#fff"
                    rx="5.625"
                  />
                  <g
                    fill="#16216a"
                    fillRule="evenodd"
                    clipRule="evenodd"
                  >
                    <path
                      d="M34.28 10.392a8.323 8.323 0 00-2.91-.505c-3.205 0-5.465 1.624-5.484 3.954-.019 1.72 1.612 2.68 2.843 3.254 1.263.587 1.69.961 1.683 1.485-.009.803-1.01 1.169-1.944 1.169-1.295 0-1.985-.182-3.052-.628l-.418-.189-.456 2.681c.756.334 2.159.624 3.615.638 3.412 0 5.626-1.605 5.651-4.093.011-1.36-.851-2.398-2.724-3.253-1.135-.555-1.83-.924-1.825-1.485 0-.497.591-1.03 1.86-1.03a5.955 5.955 0 012.429.458l.29.139zm8.318-.284h-2.506c-.777 0-1.358.214-1.698.992l-4.82 10.977h3.408s.556-1.476.683-1.799l4.155.005c.097.419.395 1.794.395 1.794h3.011zm-4.003 7.721c.27-.692 1.293-3.35 1.293-3.35-.017.033.267-.693.43-1.144l.22 1.033.753 3.461zm-22.051-7.725l-3.176 8.168-.34-1.66c-.589-1.911-2.433-3.986-4.493-5.024l2.903 10.474 3.434-.003 5.11-11.954z"
                    />
                    <path
                      d="M10.42 10.098H5.184l-.04.248c4.07.991 6.766 3.389 7.882 6.267l-1.137-5.505c-.197-.759-.765-.983-1.47-1.01zm8.845 11.974l2.028-11.976h3.246l-2.032 11.976z"
                    />
                  </g>
                </svg>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                  viewBox="0 0 53 33"
                  className="object-contain w-auto h-10"
                >
                  <radialGradient
                    id="aa"
                    cx="0"
                    cy="0"
                    r="1"
                    gradientTransform="matrix(20.11486 22.13798 -27.9023 25.3524 11 5.458)"
                    gradientUnits="userSpaceOnUse"
                  >
                    <stop
                      offset="0"
                      stopColor="#65bcf2"
                    />
                    <stop
                      offset="1"
                      stopColor="#004fc5"
                    />
                  </radialGradient>
                  <mask
                    id="bb"
                    width="53"
                    height="33"
                    x="0"
                    y="0"
                    maskUnits="userSpaceOnUse"
                  >
                    <path
                      fill="#fff"
                      fillRule="evenodd"
                      d="M2 0a2 2 0 00-2 2v29a2 2 0 002 2h48.319c1.105 0 2-.89 2-1.995V1.995a1.996 1.996 0 00-2-1.995z"
                      clipRule="evenodd"
                    />
                  </mask>
                  <g
                    fillRule="evenodd"
                    clipRule="evenodd"
                  >
                    <path
                      fill="url(#aa)"
                      d="M6 0a6 6 0 00-6 6v21a6 6 0 006 6h40.319c3.314 0 6-2.685 6-5.999V5.999a5.998 5.998 0 00-6-5.998z"
                    />
                    <g mask="url(#bb)">
                      <path
                        fill="#fff"
                        d="M13.544 17.005h.664l.19-.424v.424h10.803v-1.943h.364l.122.001c.002.051 0 .102 0 .142v.112l.012 1.688h5.453v-.577c.732.473 1.632.577 2.165.577h2.2l.481-1.193h1.309l.502 1.193h4.326l-.002-1.382.842 1.382h3.423v-7.38h-3.36v1.096l-.666-1.097H38.81v1.125l-.484-1.125H33.41c-.943 0-1.684.198-2.259.588v-.588h-3.357v.337c-.603-.3-1.237-.337-1.938-.337h-11.94l-.913 2.137-.99-2.137H8.232v1.146l-.495-1.146H4.474l-3.177 7.38h3.63l.498-1.192H6.74l.493 1.193h4.326v-.426l.192.426zm17.614-5.336c.085-.141.18-.277.302-.404.418-.428 1.069-.626 1.956-.626h1.245v1.146h-1.216c-.47 0-.74.071-.996.318-.216.228-.368.652-.368 1.214 0 .575.11.985.356 1.257.194.21.556.275.895.275h.58l1.814-4.21h1.93l2.18 5.062V10.64h1.966l2.263 3.727V10.64h1.318v5.353h-1.826l-2.441-4.014.002 4.014h-2.623l-.502-1.195h-2.68l-.482 1.195h-1.51c-.627 0-1.42-.14-1.866-.59-.457-.46-.69-1.07-.69-2.047 0-.619.094-1.192.393-1.686zm-2.342 2.049v-3.08h1.314v5.354h-1.314zm-5.925-3.08h2.963c.655 0 1.143.021 1.56.26.408.237.652.587.652 1.186 0 .373-.12.657-.273.882-.195.285-.447.47-.634.544.281.106.521.292.634.451.182.262.217.505.217.977v1.054h-1.3l-.005-.675c0-.318.033-.784-.207-1.039-.188-.19-.471-.23-.936-.23h-1.386v1.944h-1.285zm-5.192 0h4.303v1.12H18.99v.96h2.94v1.1l-2.94-.004v1.07h3.013v1.108H17.7zm-7.162 1.157v4.197H7.918l-.493-1.195H4.741l-.497 1.195h-1.4l2.303-5.353h1.917l2.188 5.07v-5.07h2.107l1.683 3.633 1.55-3.633h2.145v5.353h-1.319l-.007-4.188-1.867 4.188h-1.13z"
                      />
                      <path
                        fill="#fff"
                        d="M5.055 13.516H6.83l-.89-2.013zm21.459-.908a.491.491 0 00.272-.462.439.439 0 00-.273-.425c-.158-.077-.398-.084-.63-.084h-1.625v1.07h1.603c.26 0 .474-.009.653-.099zm10.103-1.105l-.883 2.013h1.777zM22.683 24.587v-4.362L20.77 22.37zm2.82-3.892h-1.722v1.476h1.709c.506 0 .82-.261.82-.764 0-.51-.33-.712-.807-.712zm-11.089 0v.936h2.792v1.054h-2.792v1.03h3.126l1.452-1.516-1.391-1.504z"
                      />
                      <path
                        fill="#fff"
                        d="M49.472 21.85h.145v-3.168h-3.59c-.603 0-1.093.139-1.498.353v-.353h-3.59c-.569 0-1.038.123-1.43.316v-.316H33.19v.326c-.618-.289-1.227-.326-1.897-.326h-3.977v.348c-.437-.212-1.005-.348-1.748-.348h-4.541l-1.175 1.262-1.136-1.262h-6.63v7.38h6.541l.303-.325.88-.95 1.138 1.276h3.963v-1.795h.583c.784 0 1.368-.146 1.822-.367v2.162h3.321v-1.956h.356l.131.002c.002.059 0 .118 0 .163v1.791h10.632c.61 0 1.13-.115 1.57-.313v.313h3.518c1.898 0 3.031-1.012 3.031-2.705 0-.61-.13-1.1-.403-1.509zM26.98 22.94c-.038.02-.081.039-.122.058-.09.044-.184.082-.283.114-.05.014-.098.028-.15.04a2.921 2.921 0 01-.32.059c-.047.006-.091.015-.14.02a5.25 5.25 0 01-.471.023h-1.6v1.795h-2.49l-1.58-1.77-1.642 1.77H13.1v-5.353h5.16l1.577 1.752 1.63-1.752h4.099c.143 0 .29.008.437.022.043.004.084.011.127.018a3.148 3.148 0 01.449.095c.102.03.197.07.291.116.036.017.073.031.106.051.124.071.239.154.338.259.253.266.412.651.412 1.204 0 .77-.303 1.22-.748 1.478zm6.24.071c.181.263.21.506.213.98v1.058h-1.286v-.67c0-.323.03-.797-.208-1.046-.187-.196-.473-.24-.94-.24h-1.373v1.956H28.34v-5.353h2.958c.651 0 1.126.027 1.545.257.139.082.252.183.352.297.187.216.304.491.304.89 0 .851-.57 1.287-.909 1.42.139.05.26.12.365.197.115.082.207.172.263.254zm5.275-3.12v.915H35.49v.97h2.93v1.091h-2.93v1.066l3.004.005v1.11h-4.283v-5.352h4.283zm4.835 4.638a1.34 1.34 0 01-.239.186 1.78 1.78 0 01-.41.202c-.063.019-.133.032-.2.047a3.09 3.09 0 01-.721.085h-2.5V23.9h2.49c.247 0 .416-.031.524-.133a.46.46 0 00.152-.347.443.443 0 00-.159-.354c-.098-.085-.231-.12-.455-.122-.197-.006-.403-.012-.609-.02a10.881 10.881 0 00-.074-.004c-.44-.021-.873-.078-1.228-.242H39.9a1.33 1.33 0 01-.463-.347c-.198-.238-.327-.566-.327-1.043s.188-.973.649-1.28l.002-.003c.087-.057.183-.106.288-.149l.033-.016c.098-.038.21-.065.324-.089.024-.004.044-.013.069-.018.143-.024.298-.036.469-.036h2.572v1.138h-2.352c-.235 0-.387.01-.516.096-.14.088-.191.218-.191.388 0 .2.12.337.28.397.134.047.278.06.497.062l.693.018c.162.004.311.015.448.032.126.015.238.038.345.063.008.001.017.002.024.005.27.067.488.171.645.328.254.261.385.587.385 1.134 0 .518-.165.895-.444 1.17zm3.52.52h-2.501V23.9h2.49c.247 0 .415-.031.524-.133a.459.459 0 00.152-.347.443.443 0 00-.158-.354c-.098-.085-.231-.12-.455-.122-.186-.006-.379-.011-.572-.019a6.641 6.641 0 01-.63-.053l-.008-.001c-.81-.112-1.492-.445-1.492-1.583 0-.347.106-.699.335-.985.067-.084.146-.161.236-.232l.02-.017c.083-.062.176-.115.279-.162.02-.01.038-.022.058-.03.097-.041.205-.072.32-.097.033-.008.06-.019.095-.025a2.77 2.77 0 01.49-.043h2.573v1.138h-2.352c-.236 0-.387.01-.517.096-.138.088-.19.218-.19.388a.41.41 0 00.28.397c.134.047.278.06.497.062l.692.018c.698.019 1.173.138 1.463.428.253.261.385.587.385 1.134 0 1.153-.722 1.69-2.014 1.69z"
                      />
                      <path
                        fill="#fff"
                        d="M31.974 20.78c-.156-.082-.394-.085-.628-.085h-1.627v1.08h1.606c.255 0 .475-.009.649-.098a.509.509 0 00.273-.463.453.453 0 00-.273-.434z"
                      />
                      <g fill="#2584ba">
                        <path
                          d="M27.191 13.267zm-1.276 9.776zm-.36-3.354zm.976 3.354zm.306-.133zm-.259-3.221zm-.34 0zm.821.201zm16.206 4.428zm-2.812-4.596zm2.3 4.998c-.056.019-.122.028-.183.043.062-.014.126-.025.183-.042zM39.7 20.023zm2.06 2.77c-.18-.005-.369-.01-.557-.018l.557.018zm-.595-.001c-.402-.022-.8-.07-1.122-.218.324.147.72.198 1.122.218zm-.985-3.024zm2.18 1.934zm-2.515.781zm7.209.309c-.17-.006-.346-.009-.523-.017.177.007.353.011.523.017zm-2.164-2.756zm.684-.309zm-.82.506zm1.553 2.587zm-1.143-3.045zm-12.23.115z"
                        />
                      </g>
                    </g>
                  </g>
                </svg>
              </div>
              <h4 className="inline-block text-sm font-semibold text-blue-500">Guaranteed safe checkout</h4>
              <div className="pt-px pl-px overflow-hidden">
                trustlock
              </div>
              <div
                data-min-width="1025"
                data-locale="en-GB"
                data-template-id="53aa8807dec7e10d38f59f32"
                data-businessunit-id="53a837e0000064000578f8ba"
                data-style-height="100px"
                data-style-width="150px"
                data-tags="en"
                data-theme="light"
                data-stars="5"
                className="trustpilot"
              >
                trust
              </div>
              <div className="text-sm text-gray-700 recaptcha">
                <p>
                  This site is protected by reCAPTCHA
                  <a
                    href="https://policies.google.com/privacy"
                    target="_blank"
                    rel="noreferrer noopener"
                  >
                    Privacy
                    Policy
                  </a>
                  {' '}
                  and
                  <a
                    href="https://policies.google.com/terms"
                    target="_blank"
                    rel="noreferrer noopener"
                  >
                    Terms
                    of Service
                  </a>
                  {' '}
                  apply.
                </p>
              </div>
            </div>
          </div>
          <div className="flex items-center py-3 space-x-4 border-t border-gray-200">
            <a href="/">
              <svg
                id="logo"
                xmlns="http://www.w3.org/2000/svg"
                fill="currentColor"
                viewBox="0 0 124 25"
                data-testid="logo"
                className="text-blue-500 h-6 lg:h-6"
              >
                <title>Zizoo</title>
                <path
                  d="M52 23.819v-3.405l9.14-14.463h-8.748V2.184h13.679v3.34L56.9 20.052h9.17v3.768H52zm16.545 0V2.183h4.179V23.82h-4.179zm6.453 0v-3.405l9.139-14.463h-8.75V2.184h13.68v3.34l-9.171 14.527h9.172v3.768H74.997zm29.342-2.19C102.837 23.15 101.002 24 98.567 24c-2.435 0-4.298-.852-5.802-2.37-2.166-2.187-2.106-4.89-2.106-8.628 0-3.74-.06-6.442 2.106-8.63C94.268 2.85 96.132 2 98.567 2c2.435 0 4.27.852 5.772 2.371 2.164 2.19 2.135 4.891 2.135 8.63 0 3.738.03 6.442-2.135 8.63v-.002zm-3.097-14.704c-.601-.698-1.534-1.156-2.676-1.156-1.142 0-2.104.457-2.707 1.155-.81.912-1.02 1.912-1.02 6.075s.21 5.165 1.02 6.076c.603.698 1.565 1.154 2.707 1.154s2.075-.456 2.677-1.153c.812-.912 1.053-1.913 1.053-6.076 0-4.164-.24-5.166-1.053-6.078l-.001.003zm20.624 14.705c-1.503 1.518-3.336 2.37-5.772 2.37-2.436 0-4.3-.852-5.804-2.37-2.165-2.187-2.107-4.89-2.107-8.628 0-3.74-.058-6.442 2.107-8.63C111.794 2.85 113.658 2 116.093 2c2.437 0 4.271.852 5.772 2.371 2.165 2.19 2.135 4.891 2.135 8.63 0 3.738.03 6.442-2.135 8.63h.002zm-3.098-14.705c-.6-.698-1.53-1.156-2.675-1.156-1.141 0-2.105.457-2.705 1.155-.813.912-1.023 1.912-1.023 6.075s.21 5.165 1.023 6.076c.6.698 1.562 1.154 2.703 1.154 1.145 0 2.076-.456 2.675-1.153.814-.912 1.055-1.913 1.055-6.076 0-4.164-.24-5.166-1.055-6.078h.002v.003zM27.49 14.89l8.574 8.584L34.542 25h-.001l-8.574-8.584L17.395 25l-1.525-1.526 8.574-8.584L11.096 1.526 12.618 0l13.35 13.365L39.316.002l1.522 1.523L27.49 14.89zM42.362 3.051l-9.64 9.65 9.64 9.653L52 12.702l-9.64-9.653h.001v.002zm-.722 15.177l-4.796-4.387h4.797v4.387h-.001zm0-6.441h-4.797l4.796-4.61v4.61zm1.64-4.61l4.602 4.61h-4.603v-4.61zm0 11.05v-4.386h4.602l-4.603 4.387zm-25.14-6.44h-.001l1.093 1.139-9.622 9.427L0 12.703 9.697 3.05l1.196 1.078v7.658h7.246zm-8.56 7.402v.002l5.596-5.35H8.757V7.225l-5.604 5.423 6.425 6.542z"
                />
              </svg>
            </a>
            <span className="text-xs text-blue-500">© 2014—2022 Zizooboats GmbH All rights reserved</span>
          </div>
        </div>
      </footer>
    </>
  );
}
